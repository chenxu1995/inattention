function par = exp_config_logit_test()
...
par.x0 = 40+round(5*(-1+2*rand));    % the initial signal strength
par.x_lim = [-10 50];   % the limits to the signal strength

par.alpha = struct(...
    'limits',[-10 50],...       %range of the parameter space for alpha
...
par.beta = struct(...
    'limits',[.5 10],...      %range of the parameter space for beta
    'N',1,...                %number of beta values. If this value is set to 1, then the first element of beta.limits would be the assumed beta and the beta parameter is not estimated.
...
par.lambda = struct(...
    'limits',[0 0.5],...      %range of the parameter space for lambda
